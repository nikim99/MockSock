...
def drawLine(canvas, data):
    for pairNum in range(0, len(data.linePoints2), 4):
        x1 = data.linePoints2[pairNum]
        y1 = data.linePoints2[pairNum + 1]
        x2 = data.linePoints2[pairNum + 2]
        y2 = data.linePoints2[pairNum + 3]
        canvas.create_line(x1, y1, x2, y2, width = 3)

def redrawAll(canvas, data):
    canvas.create_image(data.width/2, data.height/2, image = data.curtains)
    canvas.create_image(data.width/2, data.height/10, image = data.customize2)
    #Color
